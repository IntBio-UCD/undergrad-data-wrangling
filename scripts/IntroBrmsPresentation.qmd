...
## Check the number of "cores" on our computer {.notincremental}
...
## Fit a model with height being predicted by pop {.notincremental}
...
## Getting posterior probability for an estimate {.notincremental}
...
```{r, echo=TRUE}
f1 <- brmsformula(height_cm ~ Hmax - (Hmax - Hmin) * exp(-(k * day)^delta),
                  Hmax + Hmin + k + delta ~ 1,
                  nl=TRUE)
```
...
* The notation `Hmax + Hmin + k + delta ~ 1` means that we want a single parameter value for each of the four parameters
...
```{r}
get_prior(f1, data=dat)
```
## Set the priors

What priors should we use?

Once we decide we can set them like this

```{r echo=TRUE. eval=FALSE}
prior1 <- c(set_prior("normal(10,10)", lb=0, nlpar="Hmin"),
            set_prior("normal(...
```

```{r echo=TRUE, eval=FALSE}
prior1 <- c(set_prior("normal(10,10)", lb=0, nlpar="Hmin"),
            set_prior("normal(60,20)", lb=0, nlpar="Hmax"),
            set_prior("normal(0.01, 0.03", lb=0, nlpar = "k"),
            set_prior("normal(7,5)", lb=0, nlpar="delta")
)
```

```{r}
fit1 <- brm(formula=f1, data=dat, prior=prior1, chains = 4)
```
...
```
```{r}
dat$pred1 <- predict(fit1)[,"Estimate"]

dat %>% ggplot(aes(x=day)) +
